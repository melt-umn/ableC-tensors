...
//same as able_context_map_test.xc file except this one calls map_with_context_cilk
typedef struct {
  float to_add;
} FloatToAdd;
...
Tensor add_first_to_all(Tensor ten) {
  ...
  map_with_context_cilk(add_to_elem,ten,&elem_to_add); //function,tensor,context
  return ten;
}
